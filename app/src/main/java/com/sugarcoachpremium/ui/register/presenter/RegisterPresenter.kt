...
class RegisterPresenter<V : RegisterView, I : RegisterInteractorImp> @Inject internal constructor(
    interactor: I,
    ...
    private fun getTreatmentHorario() {
        ...
        interactor?.let {
            compositeDisposable.add(
                it.getTreatmentHorarios(dailyRegister.category_id)
                    .compose(schedulerProvider.ioToMainSingleScheduler())
                    .subscribe({ horario ->
                        if (horario.selected) {
    ...
    private fun checkIfRegisterExists(
        categoryId: Int,
        date: LocalDateTime,
                ...
                if (glucose == 0f || insulin == 0f ||
                    ...
                    interactor?.let {
                        CoroutineScope(Dispatchers.IO).launch {
                            compositeDisposable.add(
                                it.saveRegisterCall(dailyRegister)
                                    .compose(schedulerProvider.ioToMainObservableScheduler())
                                    .subscribe({ response ->
                                        if (response.id != "") {
                                            if (photo.isNotEmpty()) {
    ...
    private fun updatePoints(points: Int) {
        interactor?.let { inte ->
            compositeDisposable.add(
                inte.updateLocalPoints(user, points)
                .compose(schedulerProvider.ioToMainObservableScheduler())
    ...
    private fun cantParametersChanged(): Int {
        ...
        val listOfProperties =
            mutableListOf("glucose", "insulin", "carbohydrates", "emotionalState", "exercise")
        ...
        for (property in properties) {
            ...
            if ((valor != "" && valor != null) && (property.name in listOfProperties)) {
                Log.i(
                    "OnRegisterPresenter",
                    "cantParameterChanged: El valor es: $valor, ${property.name}"
                )
    ...
    fun uploadPhoto(id: String?, photo: File, dailyRegister: DailyRegister) {
        interactor?.let {
            compositeDisposable.add(
                it.saveRegisterPhotoCall(id.toString(), photo)
                .compose(schedulerProvider.ioToMainObservableScheduler())
    ...
    private fun getMedition(dailyRegister: DailyRegister?, category: List<Category>) {
        ...
        val breakfastId =
            category.filter { category -> category.cate_name.equals("register_breakfast_label") }
                .single().cate_id
        val pbreakfastId =
            category.filter { category -> category.cate_name.equals("register_pbreakfast_label") }
                .single().cate_id
        val lunchId =
            category.filter { category -> category.cate_name.equals("register_lunch_label") }
                .single().cate_id
        val plunchId =
            category.filter { category -> category.cate_name.equals("register_plunch_label") }
                .single().cate_id
        val snackId =
            category.filter { category -> category.cate_name.equals("register_snack_label") }
                .single().cate_id
        val psnackId =
            category.filter { category -> category.cate_name.equals("register_psnack_label") }
                .single().cate_id
        val dinnerId =
            category.filter { category -> category.cate_name.equals("register_dinner_label") }
                .single().cate_id
        val pdinnerId =
            category.filter { category -> category.cate_name.equals("register_pdinner_label") }
                .single().cate_id
        ...
        if (date == null) {
            when {
                ...
                timeBetween(
                    ...
                    dinner,
                    dinnerEnd
                ) || timeBetween(currentDate.toLocalTime(), midnight, midnightEnd) -> {
                    index = dinnerId
                    getView()?.setDateMedition(currentDate.toDate(), index, category)
                }
            ...
            when {
                dailyRegister?.category_id == breakfastId && timeBetween(
                    currentDate.toLocalTime(),
                    breakfast,
                    breakfastEnd
                ) -> {
                ...
                (dailyRegister?.category_id == breakfastId || dailyRegister?.category_id == pbreakfastId) && timeBetween(
                    currentDate.toLocalTime(),
                    breakfast,
                    breakfastEnd
                ) -> {
                ...
                dailyRegister?.category_id == lunchId && timeBetween(
                    currentDate.toLocalTime(),
                    lunch,
                    lunchEnd
                ) -> {
                ...
                (dailyRegister?.category_id == lunchId || dailyRegister?.category_id == plunchId) && timeBetween(
                    currentDate.toLocalTime(),
                    lunch,
                    lunchEnd
                ) -> {
                ...
                dailyRegister?.category_id == snackId && timeBetween(
                    currentDate.toLocalTime(),
                    snack,
                    snackEnd
                ) -> {
                ...
                (dailyRegister?.category_id == snackId || dailyRegister?.category_id == psnackId) && (timeBetween(
                    currentDate.toLocalTime(),
                    snack,
                    snackEnd
                )) -> {
                ...
                dailyRegister?.category_id == dinnerId && (timeBetween(
                    currentDate.toLocalTime(),
                    dinner,
                    dinnerEnd
                ) || timeBetween(currentDate.toLocalTime(), midnight, midnightEnd)) -> {
                ...
                (dailyRegister?.category_id == dinnerId || dailyRegister?.category_id == pdinnerId) && timeBetween(
                    currentDate.toLocalTime(),
                    dinner,
                    dinnerEnd
                ) -> {
    ...
    fun timeBetween(time: LocalTime, startTime: LocalTime, endTime: LocalTime): Boolean {
        return time.isAfter(startTime) && time.isBefore(endTime) || time.isEqual(startTime) || time.isEqual(
            endTime
        )
    }
    ...
    override fun setTime(hourOfDay: Int, minute: Int) {
        ...
        var medition = 0
        when {
            timeBetween(date.toLocalTime(), breakfast, breakfastEnd) -> medition =
                category.filter { category -> category.cate_name.equals("register_breakfast_label") }
                    .single().cate_id

            timeBetween(date.toLocalTime(), lunch, lunchEnd) -> medition =
                category.filter { category -> category.cate_name.equals("register_lunch_label") }
                    .single().cate_id

            timeBetween(date.toLocalTime(), snack, snackEnd) -> medition =
                category.filter { category -> category.cate_name.equals("register_snack_label") }
                    .single().cate_id

            timeBetween(date.toLocalTime(), dinner, dinnerEnd) || timeBetween(
                date.toLocalTime(),
                midnight,
                midnightEnd
            ) -> medition =
    ...
    override fun showTimeDialog(
        ...
        var tpd: TimePickerDialog =
            TimePickerDialog.newInstance(timeSetListener, date.hourOfDay, date.minuteOfHour, true)
